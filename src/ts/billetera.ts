...
function renderizarBilletera(): void {
  const billeteraContenedor = document.getElementById(
    "billetera-contenedor"
  ) as HTMLElement | null;
  if (!billeteraContenedor) return;

  billeteraContenedor.innerHTML = "";

  const billetera = JSON.parse(localStorage.getItem("billetera") || "{}") as {
  ...
  Object.entries(billetera).forEach(
    ([id, data]: [string, { count: number; img: string }]) => {
      if (data.count > 0) {
